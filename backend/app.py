...
#这里开始修改    
    def design_butterfly(self, forecast_price, price_stability, volatility, iv_skew, dominant_period, butterfly_type='CALL'):
        ...
        butterfly_type: 'CALL' 或 'PUT' 或 'IRON'
        ...
        # *** 关键修复: 根据butterfly_type选择期权类型 ***
        if butterfly_type == 'IRON':
            # 铁蝴蝶: 下翼用PUT, 上翼用CALL, 中心两个都用
        ...
        # 计算Greeks (铁蝴蝶需要特殊处理)
        if butterfly_type == 'IRON':
            # 铁蝴蝶的Greeks = PUT spread Greeks + CALL spread Greeks
            # 简化: 使用组合Greeks
        ...
        return {
            ...
            'is_credit_strategy': is_credit_strategy,  # 新增: 标识是否为收入策略
...
#修改到此处为止
    ...
    def full_analysis(self):
        ...
        butterfly = self.design_butterfly(
            ...
            fourier_result['butterfly_type']  # 新增参数
        )
