...
// Extend JSX elements to include the custom TradingView element
declare global {
  namespace JSX {
...
const ButterflyOptionAnalyzer: React.FC = () => {
  ...
  const pageTitle = (() => {
    if (results && ticker.toUpperCase() === analyzedTicker) {
      // 3. 分析完成：标题变成 "TSLA 蝶式期权分析 | ButterQuantDL分析器 v2.1"
      return `${analyzedTicker} 蝶式期权分析 | ButterQuantDL v2.1`;
    } else if (ticker) {
      // 2. 用户输入 "TSLA"：标题立刻变成 "TSLA - ButterQuantDL分析器 v2.1"
      return `${ticker.toUpperCase()} - ButterQuantDL分析器 v2.1`;
    } else {
      // 1. 默认打开页面：标题是 "ButterQuantDL分析器 v2.1"
          ...
          <>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
              ...
              <div className="mt-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
                <h3 className="font-semibold text-purple-900 mb-2">🔍 频域解读</h3>
                <ul className="text-sm text-purple-800 space-y-1">
                  <li>• <strong>红色曲线 (低频)</strong>: 长期趋势，当前{results.fourier.trend_direction === 'UP' ? '向上' : results.fourier.trend_direction === 'DOWN' ? '向下' : '平稳'}</li>
                  <li>• <strong>绿色虚线 (中频)</strong>: 周期性波动，当前处于{results.fourier.cycle_position === 'PEAK' ? '波峰' : '波谷'}位置</li>
                  <li>• <strong>蓝色曲线</strong>: 原始价格 = 低频 + 中频 + 高频噪声</li>
                  <li>• <strong>策略逻辑</strong>: {results.fourier.strategy_reason}</li>
                  {/* 🆕 显示周期强度 */}
                  {results.fourier.period_strength && (
                    <li>• <strong>周期强度</strong>: {results.fourier.period_strength.toFixed(1)}%
                      {results.fourier.period_strength > 10 ? ' (周期明显)' : ' (周期较弱)'}
                    </li>
                  )}
                </ul>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
              ...
              <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <ul className="text-sm text-blue-800 space-y-1">
                  ...
                  <li>• <strong>风险提示</strong>：如果未来价格突破置信区间，需立即止损</li>
                  {/* 🆕 显示预测准确性 */}
                  {results.arima.confidence_width && (
                    <li>• <strong>预测区间宽度</strong>：${results.arima.confidence_width.toFixed(2)}
                      {results.arima.confidence_width / results.forecast_price < 0.10 ? ' (窄，信心高)' : ' (宽，不确定性大)'}
                    </li>
                  )}
                </ul>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
              ...
              <div className="mt-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
                <h3 className="font-semibold text-purple-900 mb-2">📊 波动率分析</h3>
                <ul className="text-sm text-purple-800 space-y-1">
                  <li>• <strong>当前隐含波动率 (IV)</strong>: {(results.garch.current_iv * 100).toFixed(1)}%</li>
                  <li>• <strong>GARCH预测波动率</strong>: {(results.garch.predicted_vol * 100).toFixed(1)}%</li>
                  <li>• <strong>历史波动率</strong>: {(results.garch.historical_vol * 100).toFixed(1)}%</li>
                  <li>• <strong>波动率溢价</strong>: {results.garch.vol_mispricing > 0 ? '+' : ''}{results.garch.vol_mispricing.toFixed(1)}% （IV {results.garch.vol_mispricing > 0 ? '被高估' : '被低估'}）</li>
                  {/* 🆕 显示IV百分位 */}
                  {results.garch.iv_percentile && (
                    <li>• <strong>IV历史百分位</strong>: {results.garch.iv_percentile.toFixed(0)}%
                      {results.garch.iv_percentile > 75 ? ' (高位，适合卖期权)' :
                        results.garch.iv_percentile < 25 ? ' (低位，不适合卖期权)' : ' (中等)'}
                    </li>
                  )}
                  <li>• <strong>交易机会</strong>: {results.garch.vol_mispricing > 10 ? '做多蝴蝶期权可赚取波动率溢价' : '波动率溢价不明显'}</li>
                </ul>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                  <p className="text-sm text-gray-600 mb-1">下翼 (买入)</p>
                ...
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <p className="text-sm text-gray-600 mb-1">中心 (卖出2份)</p>
                ...
                <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                  <p className="text-sm text-gray-600 mb-1">上翼 (买入)</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-red-50 rounded-lg border border-red-200">
                  ...
                  <p className="text-3xl font-bold text-red-600">${results.butterfly.net_debit.toFixed(2)}</p>
                  <p className="text-sm text-red-700 mt-1">净权利金支出（初始成本）</p>
                  {/* 🆕 显示最大损失 */}
                ...
                <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                  ...
                  <p className="text-3xl font-bold text-green-600">${results.butterfly.max_profit.toFixed(2)}</p>
                  <p className="text-sm text-green-700 mt-1">价格在${results.butterfly.center_strike.toFixed(0)}时实现</p>
                  {/* 🆕 显示盈利概率 */}
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className={`mt-4 p-4 rounded-lg border-2 ${Object.values(results.signals).filter(s => s).length >= Math.ceil(Object.keys(results.signals).length * 0.7)
