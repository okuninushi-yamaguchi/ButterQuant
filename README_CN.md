...
## 使用之前请务必注意，v4.1的更新的一些说明
v4.1添加了每日扫描的功能，会自动扫描Nasdaq 100和S&P 500的成分股，将结果存入SQLite数据库，扫描过程可能需要较长时间（30分钟+），扫描结果的简单展示在前端仪表盘中，点击任意股票代码，即可自动跳转至分析页并开始深度分析（Click-to-Analyze）。扫描过程的全部结果（包含中间大量计算结果）会写入另一个SQLite数据库中，每日启动一次扫描，将会成为历史数据，不断积累。目前的估计，每一天的数据大概是23MB左右，一年的数据大概是7GB左右。使用该功能前请务必确认文档的储存路径，和硬碟容量。若无法满足，可以不使用该功能，则前端仪表盘将无法显示扫描结果。但不影响其他功能的使用。
储存的数据，后期项目作者会开发一个深度学习的程序，进行分析。数据写入的标准亦为此提起规定好。若想后期使用深度学习，要确保每天数据的写入无误。
